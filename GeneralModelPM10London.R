...
## station ID 는 뭐 해야햄.
bulk_london <- read.csv("bulk.csv") %>% filter(stationId == 2643743)
...
raw <- inner_join(df_aq_simple, bulk_london, by=c("utc_time" = "date"))
...
#added
raw_onehot <- cbind(raw_w, onehot_h,onehot_m, onehot_weather, onehot_day) %>% select(-stationId.x,-X,-stationId.y,-weather_code, -weather_desc, -weather_cloud, -weather_rain, -utc_time, -NO2, -CO, -O3, -SO2, -weather, -y, -m, -d, -h, -day) %>% mutate(humidity = as.numeric(humidity))
raw_onehot <- raw_onehot[!is.na(raw_onehot$PM10),] # NULL값 제거
...
xgb.save(xmodel, "xgbII_model_london_general_pm10")
