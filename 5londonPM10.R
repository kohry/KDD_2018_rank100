...
city_list <- c('BX9',	'BX1',	'BL0',	'CD9',	'CD1','CT2'	,'CT3',	'CR8',	'GN0',	'GR4',	'GN3',	'GR9',	'GB0',	'HR1',	'HV1',	'LH0',	'KC1',	'KF1')
...
while (iii <= length(city_list)) {
  ...
  while (iilag <= 55) {
    ...
    raw_w$lag <- lag(raw_w$PM10,iilag)
    raw_w <- raw_w %>% drop_na(PM10, lag)
    ...
    #added
    raw_onehot <- cbind(raw_w, onehot_h,onehot_m, onehot_weather, onehot_day) %>% select(-stationId.x,-X,-stationId.y,-weather_code, -weather_desc, -weather_cloud, -weather_rain, -utc_time, -PM2.5, -NO2, -CO, -O3, -SO2, -weather, -y, -m, -d, -h, -day) %>% mutate(humidity = as.numeric(humidity))
    ...
    tryCatch({
      ...
      xgb.save(xmodel, paste("xgbII_model_london_PM10",iii,iilag,sep="_"))
  ...
  iii = iii + 1
  write.csv(smapes, "xgbII_model_london_smapes_PM10.csv") ##RRRR
