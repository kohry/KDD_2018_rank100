...
# forecast stretch, becuase forecast data interval is 3 hours.
...
# this is needed becuase we have only 3 hour interval weather data, so we have to fill the gap between 2 hours. if received 00 03, we have to fill out 01, 02
...
raw_w$day <- wday(raw_w$utc_time, label = T)
...
for (icity in 1:length(city_list_china)) {
  ...
  #level diff exists. this is not the andswer! becuase levels info is distorted when just appplying levels() function to the different level
  ...
  weathers <- d2$weather
  # we take out the levels, and reallocation this.
  for(aax in 1:48) {
  ...
  d4_onehot$m01 <- 0 ##RR this is just a temporary reallocation becuase due to level difference
  d4_onehot$m05 <- 1 ##RR
...
for (icity in 36:48) {
  ...
  for(iter in 1:48) {
    ...
    str = "xgbII_model_london_general" #Only difference between original version and pre version to check the diff
...
###################################################################
forecasting_file <- "/root/current_submission.csv"
