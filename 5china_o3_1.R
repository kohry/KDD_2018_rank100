...
while (iii <= 18) {
  ...
  while (iilag <= 55) {
    ...
    raw_w$lag <- lag(raw_w$O3,iilag)
    raw_w <- raw_w %>% drop_na(O3, lag)
    ...
    #added
    raw_onehot <- cbind(raw_w, onehot_h,onehot_m, onehot_weather, onehot_day) %>% select(-stationId.x,-X,-stationId.y,-weather_code, -weather_desc, -weather_cloud, -weather_rain, -utc_time, -PM2.5, -PM10, -NO2, -CO,  -SO2, -weather, -y, -m, -d, -h, -day) %>% mutate(humidity = as.numeric(humidity))
    ...
    tryCatch({
      ...
      xgb.save(xmodel, paste("xgbII_model_china_O3",iii,iilag,sep="_"))
  ...
  iii = iii + 1
  write.csv(smapes, "xgbII_model_china_smapes_O3_1.csv") ##RRRR
